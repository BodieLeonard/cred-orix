...
function register_cpt_offices() {
    ...
    $args = array( 
        'labels' => $labels,
        'hierarchical' => true,
        'description' => 'Orix Office Articles.',
        ...
        'can_export' => true,
        'rewrite' => array(
             'slug' => 'offices', // This controls the base slug that will display before each term
    ...
    add_filter('manage_'.$PLUGIN_NAME_SINGULAR_LOWER.'_posts_columns', 'manage_offices_columns');
    add_action('manage_'.$PLUGIN_NAME_SINGULAR_LOWER.'_posts_custom_column', 'print_offices_column', 10, 2);
    remove_post_type_support( 'offices', 'editor' );
};


function offices_taxonomy() {  
    register_taxonomy(  
        'officescategory',  //The name of the taxonomy. Name should be in slug form (must not contain capital letters or spaces). 
        'offices',        //post type name
        array(  
            'hierarchical' => true,  
            'label' => 'Office Categories',  //Display name
            'query_var' => true,
            'rewrite' => array(
                'slug' => 'offices', // This controls the base slug that will display before each term
                'with_front' => false // Don't display the category base before 
...
function manage_offices_columns($columns) {
  $columns['location'] = "Location";
  $columns['order'] = "Order";
  return $columns;
};

function print_offices_column($column_name, $post_id) {
  ...
  if( $column_name == 'location' ) { 
    ...
    $output = '';
    $categories = get_the_terms($post->ID, 'officescategory' );;
    if($categories){
